# Specimen Icons — Prompt & Style Guide

Paste everything below the line into a new session, then ask for the icons you want, for example: *"Draw specimens for: tuning fork, jellyfish, EMG electrode."*

---

## Your task

You are drawing **specimen icons** for joshuaanderton.ca. Each icon is a small scientific-catalogue illustration: a single subject drawn with clean outlines, flat grey fills and a few faint detail lines, like a plate in a field guide or a lab supply catalogue. Every icon must look like it came from the same hand as the existing set.

For each subject I ask for, return one JavaScript object entry that I can paste into the `ART` object:
...
- Return only the **inner markup**. The wrapper `<svg class="sp" viewBox="0 0 200 200">` is added by the site.
- After the code, give me a one-line label for each icon in Title Case (for example "Tuning Fork").
...
Colour is never written into the drawing. Every shape uses one of these classes, which the site maps to theme colours so light and dark mode both work.
...
The site sets `stroke-linecap:round` and `stroke-linejoin:round` on everything, so don't add them yourself.
...
- The **tube technique** for thick outlined bands such as a headband, padlock shackle or eel body. Draw the same path twice:
  ```html
  <path d="…" fill="none" stroke="var(--stroke)" stroke-width="16"/>
  <path d="…" fill="none" stroke="var(--tile)"   stroke-width="12"/>
  ```
  The outer width minus the inner width must equal 4, which gives a 2px outline on each side. You may add a third pass in `var(--fill)` for a grey core or a dashed pattern.
...
- Only if a letter is essential (N/S on a magnet): `<text>` with `font-family="Inter Tight,Arial,sans-serif"`, 12–16px, weight 600, `fill="var(--stroke)"`.
...
```js
lock:`<path d="M72 96 V70 a28 28 0 0 1 56 0 V96" fill="none" stroke="var(--stroke)" stroke-width="16"/>
  <path d="M72 96 V70 a28 28 0 0 1 56 0 V96" fill="none" stroke="var(--tile)" stroke-width="12"/>
...
Save this as `preview.html`, paste the entries into `ART`, and open it. Use the Dark button to check both themes.

```html
<!doctype html><meta charset="utf-8"><title>Specimen preview</title>
<style>
:root{--tile:#fff;--stroke:#3A3A3A;--fill:#D8D8D8;--bg:#C4C4C4;--ink:#1B1B1B}
.dark{--tile:#222;--stroke:#CFCFCF;--fill:#474747;--bg:#0E0E0E;--ink:#EDEDED}
body{margin:0;background:var(--bg);font:14px "Inter Tight",Arial,sans-serif;color:var(--ink);padding:24px}
.g{display:grid;grid-template-columns:repeat(auto-fill,minmax(180px,1fr));gap:1px;background:var(--fill)}
.c{background:var(--tile);padding:12px;text-align:center}.c.s svg{width:64px;height:64px}
.sp{width:100%;aspect-ratio:1;display:block}.sp *{stroke-linecap:round;stroke-linejoin:round}
.sp .a{fill:var(--tile);stroke:var(--stroke);stroke-width:2}.sp .b{fill:var(--fill);stroke:var(--stroke);stroke-width:2}
.sp .l{fill:none;stroke:var(--stroke);stroke-width:2}.sp .t{fill:none;stroke:var(--stroke);stroke-width:1.25;opacity:.55}
.sp .d{fill:var(--fill);stroke:none}.sp .k{fill:var(--stroke);stroke:none}
</style>
...
const svg = k => `<svg class="sp" viewBox="0 0 200 200">${ART[k]}</svg>`;
g.innerHTML = Object.keys(ART).map(k => `<div class="c">${svg(k)}<div>${k}</div></div><div class="c s">${svg(k)}<div>${k} @64</div></div>`).join("");
</script>
```

## Existing keys (don't reuse)

`prism` `mic` `paper` `pencil` `iron` `scope` `plug` `guitar` `desk` `pyramidal` `brain` `heart` `bacterium` `eeg` `bone` `bee` `crab` `feather` `shark` `eel` `swallow` `honeycomb` `web` `magnet` `wave` `lock` `catalog`

Add each new key to this list once it's on the site.

## Ideas queue

Some subjects that fit the set: tuning fork, jellyfish, electric ray, platypus bill, octopus, axolotl, planarian flatworm, frog embryo, neuromuscular junction, ion channel, synapse, Purkinje cell, cochlea, retina / eye, spinal cord, DNA helix, fluxgate core, Helmholtz coil, op-amp triangle, BNC connector, electrode pad, breadboard, multimeter, compass, magnifying glass, petri dish, test tube rack, microscope, turntable, reel-to-reel, speaker cone, waveform envelope.
