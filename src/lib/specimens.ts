/**
 * Specimen icons: SVG inner markup on a 200×200 canvas, wrapped by <Specimen>.
 * Drawing classes (see .sp in app.css): a = outline on white, b = outline on grey,
 * l = line only, t = faint line, d = grey fill, k = dark fill.
 * Rules for new icons: specimen-icons-prompt.md. Add each new key to its "Existing keys" list.
 */
export const ART = {
...
export function isArtKey(key: unknown): key is ArtKey {
  return typeof key === 'string' && key in ART
}
